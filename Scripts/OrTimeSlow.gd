@tool
class_name OrTimeSlow extends Node2D
...
@export var disable_bank:Dictionary[String,bool]
...
func disable_condition() -> bool:
	...
	for input in disable_bank:
		# If it's been pressed since entering the TimeSlow, disable.
		if disable_bank[input] == true: return true
	
	# IF none have been pressed, don't disable.
	return false
